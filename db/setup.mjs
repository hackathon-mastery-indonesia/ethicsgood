...
CREATE TABLE ARTICLE_CHAT_EMBEDDING(
  ...
  FOREIGN KEY(article_id) REFERENCES TO ARTICLE(id) ON UPDATE CASCADE ON DELETE CASCADE;
); 
`

const task = async () => {
  const res = await query(`ALTER TABLE ARTICLE_CHAT_EMBEDDING
  ADD CONSTRAINT fk_article_id
  FOREIGN KEY (article_id)
  REFERENCES ARTICLE(id)
  ON DELETE CASCADE
  ON UPDATE CASCADE;DELETE FROM ARTICLE; DELETE FROM ARTICLE_CATEGORY; DELETE FROM ARTICLE_EMBEDDING`)
  console.log(res)
}
